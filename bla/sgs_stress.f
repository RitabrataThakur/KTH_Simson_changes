c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/trunk/bla/sgs_stress.f $
c $LastChangedDate: 2009-11-01 14:34:05 +0100 (Sun, 01 Nov 2009) $
c $LastChangedBy: pschlatt@MECH.KTH.SE $
c $LastChangedRevision: 1442 $
c
c ***********************************************************************
      subroutine sgs_stress(taur,taui,zb,zbp,alfa,beta,
     &     gur,gui,gu3r,gu3i,w3,prey,sigma3r,sigma3i,tau3r,tau3i,
     &     gsr,gsi,ivarvisc,taurvis,tauivis,tau3rv,tau3iv)
c
...
c
c     Jose: Additional stress due to viscosity stratification
c
      integer ivarvisc
      real taurvis(memnx,memny,memnz,6), tauivis(memnx,memny,memnz,6)
      real tau3rv(nxz,nyp,6),tau3iv(nxz,nyp,6)
c
...
c
c     Jose: Additional stress due to viscous stratification
c
         if(ivarvisc.ne.0) then
            call getxy(tau3rv(1,1,i),tau3iv(1,1,i),zbp,i,taurvis,
     &           tauivis)
            call vchbf(tau3rv(1,1,i),w3,nyp,nxz,nxz,1,prey)
            call vchbf(tau3iv(1,1,i),w3,nyp,nxz,nxz,1,prey)
            do y=1,nyp
               do xz=1,nxz
                  tau3r(xz,y,i) = tau3r(xz,y,i) + c1*tau3rv(xz,y,i)
                  tau3i(xz,y,i) = tau3r(xz,y,i) + c1*tau3iv(xz,y,i)
               end do
            end do
         end if
      end do
