c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/trunk/bla/advection.f $
c $LastChangedDate: 2012-05-16 22:51:28 +0200 (Wed, 16 May 2012) $
c $LastChangedBy: pschlatt@MECH.KTH.SE $
c $LastChangedRevision: 1767 $
c
c ***********************************************************************
...
c
         do ith=1,scalar
c            write(*,*) 'In advection step: '
            if (gr(ith).ne.0.) then
c               write(*,*) 'Richardson number', gr(ith)
               do z=1,nzpc
                  do xy=1,nxy
c                     write()
                     om2r(xy,z,2) = om2r(xy,z,2) +
