c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/trunk/bla/rdiscbl.f $
...
c
         ...
         pr = 0.
         gr = 0.
c         read(12,err=2001) re,pou,xl,zl,t,xs,(pr(i),m1(i),i=1,scalar)
         read(12,err=2001)re,pou,xl,zl,t,xs,
     &        (pr(i),gr(i),m1(i),i=1,scalar),ivarvisc
         do i=1,scalar
c            gr(i) = 0.01
...
c     This loop does not occur for any permissible nz.
...
!     boxr = odd points / amplitudes
!     boxi = even points / amplitudes 
...
c
!     This seems the physical values. 
...
c
!     This doesn't look like the following loop will ever occur unless nxin
!     is different from nx.
                  do y=1,nypin
                     do x=nxin/2+1,nx/2
                        boxr(x,z-zb+1,y)=0.0
                        boxi(x,z-zb+1,y)=0.0
...
c
!     Another loop which won't be accessed. 
...
c
      if (1.eq.0) then
c     An impossible condition for now.
