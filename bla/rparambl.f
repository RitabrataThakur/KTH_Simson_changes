c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/trunk/bla/rparambl.f $
c $LastChangedDate: 2013-03-08 17:04:04 +0100 (Fri, 08 Mar 2013) $
c $LastChangedBy: taras@MECH.KTH.SE $
c $LastChangedRevision: 1818 $
c
c ***********************************************************************
...
c$$$      write(*,*)' Here in rparambl.f'
c$$$      write(*,*)' After here was the error. ' 
c$$$      write(*,*)' ########################'
c
...
c
     ...
     &           'intervals ', dsave, ' starting from time ', ssave
         ...
         do i=1,msave
            ...
            if (my_node.eq.0) then
               write(ios,*) i,tsave(i),trim(nmsave(i))
            end if
         end do
      end if
      tsave(msave+1)=1.E30
...
c
c Jose: Additional entries for varying viscosity case
c delT = The dimemsional temperature gradient      
c
      read(10,*) delT
      read(10,*) Tref
c
c     Jose: For noise to introduce for a pre formed streak
c
      nxn_nois = 0
      nyn_nois = 0
      nzn_nois = 0
...
c      write(*,*) 'Stopping here now. Jose 1.'
c      call stopnow(42001)
c
c     Close input file
c
      close(unit=10)
