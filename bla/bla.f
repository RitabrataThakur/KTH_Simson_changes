...
c
      ...
      real mu_avinv
      integer ivarvisc, suth
      integer alp_co,beta_co
c      integer alp_wn(nx/2),beta_wn(nz)
c      integer beta_loc(2*nz/3 + 1)
      real alp_fl(nx/2),beta_fl(nz)
      integer tmp
      logical jose_fl1,jose_fl2
c
c     Jose: Noise for streak
c
      logical noise
      real ed_nois
      integer nxn_nois, nyn_nois, nzn_nois
      integer seed_nois
c
...
c      real cr    (iiles*(nxp/2+1)*mby*nzd,nthread)
c      real ci    (iiles*(nxp/2+1)*mby*nzd,nthread)
c     Jose: Varying viscosity additional working arrays
c     Should I use more of these?

c
c     Jose: Temporary fields for varying viscosity
c     
      real cr((nxp/2+1)*mby,nzd), ci((nxp/2+1)*mby,nzd)
      real nur((nxp/2+1)*mby,nzd), nui((nxp/2+1)*mby,nzd)
      real tauplr((nxp/2+1)*mby,nzd), taupli((nxp/2+1)*mby,nzd)
      real taurvis(memnx,memny,memnz,6), tauivis(memnx,memny,memnz,6)
      real tau3rv(nx/2*mbz,nyp,6), tau3iv(nx/2*mbz,nyp,6)
...
c      write(*,*) 'At the start of the program: ', mbox2, mbox3
...
c
c     So the next line appears. 
c      write(*,*) 'Inside ALLTOALL if condition in MPI. line 739'
...
c$$$#ifndef WRAPPER 
c$$$c     ALLTOALL, WRAPPER are both false
c$$$      write(*,*) '###############################'
c$$$      write(*,*) 'WRAPPER is false'
c$$$      write(*,*) '###############################'
c$$$#endif
...
c
     ...
     &     jet_prof,jet_diam,xjet,zjet,jetmag,delT,Tref,noise,ed_nois,
     &     nxn_nois,nyn_nois,nzn_nois,seed_nois)
...
c
     ...
     &        w3,urx,nxtmp,my_node,gr,ivarvisc)
      end if
c
c     3D base flow
c
...
c
     ...
     &           nxtmp,my_node,gr,ivarvisc)
            ...
            if (1.eq.0) then
               write(*,*) 'Impossible condition met'
...
c
c     Jose: Manually turning on gravity and varying viscosity 
c
c      write(*,*) 'gr = ',gr
c      gr = 0.01
c      ivarvisc = 1
...
c
c     Jose: Introducing noise as Stokes modes at some point of the streak
c     evolution
      if (noise) call stnois(ur,ui,seed_nois,nxn_nois,nyn_nois,nzn_nois,
     &    ed_nois,alfa,beta,eta,boxr,boxi)
...
c
c     Jose: Choice of reference temperature for varying viscosity case.
c
c      ivarvisc = 2
c      suth = 0
c      if (ivarvisc.eq.2) then
c         suth = 1
c      endif
      suth = 0
      alp_fl = 0.0
...
c Stable stratification
         ...
         if (suth.eq.0) then
            mu_avinv = exp(-ct/Tref)
         else
            mu_avinv = (Tref+110.4)/(Tref*sqrt(Tref))
         endif
...
c     For time averaged mean streamwise flow.
      t_av = .false.
      tav_cnt = 0
      tav_u = 0.
...
c      yb = memnx*memny*memnz
c      do ybp = 1,yb
c         write(24,*) ybp, taur(ybp,1), taui(ybp,1)
c      end do

c            write(*,*) '##################'
c            write(*,*) 'Here at the end', ybp, my_node
c            write(*,*) '##################'
...
c      ivarvisc = 0
      if (ivarvisc.ne.0.and.delT.ne.0.0) then
...
c               write(*,*) '##################'
c               write(*,*) 'Before. Here at the end', ybp, my_node
c               write(*,*) '##################'
c            endif
            myb= (yb-1)/mby+1 

c               write(*,*) '##################'
c               write(*,*) 'INSIDE IF. Here at the end', ybp, my_node
c               write(*,*) '##################'               
c$$$               call varvisc_32(taurvis,tauivis,ivarvisc,yb,alfa,beta,
c$$$     &              u0low,w0low,prexn,prezn,presn,prean,prex,prez,
c$$$     &              my_node,realg1,realg2,ur,ui,
c$$$     &              re,eta,it,cr,ci,nur,nui,
c$$$     &              tauplr,taupli,s2r,s2i,l2r,l2i,m2r,m2i,
c$$$     &              om2r,om2i,du2r,du2i,u2r,u2i,delT,Tl,c1,xl,zl,suth)
c     Jose: the 2/3 dealiasing rule is less time consuming. 
c     This calculation of the additional stress due to viscosity stratification
c     takes roughly half the time step
               call varvisc_23(taurvis,tauivis,ivarvisc,yb,alfa,beta,
     &              u0low,w0low,prexn,prezn,presn,prean,
     &              my_node,realg1,realg2,ur,ui,
     &              re,eta,cr,ci,nur,nui,
     &              tauplr,taupli,s2r,s2i,l2r,l2i,m2r,m2i,
     &              om2r,om2i,du2r,du2i,u2r,u2i,delT,Tl,mu_avinv,xl,zl,
     &              alp_fl,beta_fl,suth)


c            if(my_node.eq.0) then
c               write(*,*) '##################'
c               write(*,*) 'after. Here at the end', ybp, my_node
c               write(*,*) '##################'
c            endif
...
c      do ybp = 1,yb
c         write(25,*) ybp, taur(ybp,1), taui(ybp,1)
c      end do
c      write(*,*) "Here after varvisc calculation"
c      write(*,*) 'my_node = ', my_node
c      call stopnow(910345)
...
!$omp parallel do private(zbp,zb,ip)
     ...
     &           gsr,gsi,
     &           ivarvisc,taurvis,tauivis,tau3rv,tau3iv)
         end do
...
c
     ...
     &           boxr,boxi,urx,alfa,zs,beta,my_node,gr,ivarvisc)
         end if
      end if
...
c
c     Jose: To evaluate whether entropy is to be calculated.
c
...
c
 ...
 5434       continue
            close(11)
            if (kode.eq.0) then
               write(dumpname,'(a,f9.2)')'dump_t_',t/dstar
               if (my_node.eq.0) then
...
c     
         tmp = mod(it-1,nst*100)
         jose_fl1 = (tmp.eq.0)
         jose_fl2 = (t.lt.5.0)
         if (my_node.eq.0) then
            call ctim(ctime2,wtime2)
c            if(jose_fl1.or.jose_fl2) then               
               write(ios,'(a,f10.3,a,f10.3,a)') ' CPU time: ',
...
c            if(jose_fl1.or.jose_fl2) then
               ...
               if (delta_wall_time.gt.abs(delta_wall_time_int)) then
                  delta_wall_time = 0.
                  open(file='step.out',unit=87)
                  write(87,'(a20,f18.9,a,f18.9,a)') 'Time now: ',
     &                 t/dstar,' (max. ',tmax/dstar,')'
                  write(87,'(a20,f18.9)') 'Started with: ',sttimet/dstar
                  write(87,'(a20,f18.9)') 'Delta: ',(t-sttimet)/dstar
     ...
     &                 wtime2-wtime0,
     &                 ' seconds (max. ',wallmax,')'
                  write(87,'(a,f18.9,a)') 'Last time step ',
     &                 ctime2-ctime1,' seconds (CPU time)'
                  write(87,'(a,f18.9,a)') 'Last time step ',
     &                 wtime2-wtime1,' seconds (wallclock time)'
                  close(87)
               endif
c
c     History file
