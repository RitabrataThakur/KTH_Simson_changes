c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/trunk/bls/stnois.f $
c $LastChangedDate: 2007-11-12 13:19:01 +0100 (Mon, 12 Nov 2007) $
c $LastChangedBy: mattias@MECH.KTH.SE $
c $LastChangedRevision: 850 $
c
c ***********************************************************************
      subroutine stnois(ur,ui,seed,nxn,nyn,nzn,ed,
     &     alfa,beta,eta,boxr,boxi)
c
...
c
      do i=1,3
         call getxy(boxr,boxi,1,i,ur,ui)
         do x = 1,nx/2
            do y=1,nyp
               uxy(x,y,i,1) = boxr(x,1,y) + im*boxi(x,1,y)
            enddo
         enddo
c         call getxyp(uxy(1,1,i,1),1,i,ur)
      end do
c
...
c
      ...
      do i=1,3
         do x = 1,nx/2
            do y=1,nyp
               boxr(x,1,y)=real(uxy(x,y,i,1))
               boxi(x,1,y)=aimag(uxy(x,y,i,1))
            enddo
         enddo
c         call putxy(uxy(1,1,i,1),1,i,ur)
         call putxy(boxr,boxi,1,i,ur,ui)
      end do
c
...
c
      do z=2,nzn/2+1
         ...
         zn=nz+2-z
         do i=1,3
c            call getxyp(uxy(1,1,i,1),z,i,ur)
            call getxy(boxr,boxi,z,i,ur,ui)
            do x = 1,nx/2
               do y=1,nyp
                  uxy(x,y,i,1) = boxr(x,1,y) + im*boxi(x,1,y)
               enddo
            enddo
            if (nfzsym.eq.0) then
c     call getxyp(uxy(1,1,i,2),zn,i,ur)
               call getxy(boxr,boxi,zn,i,ur,ui)
               do x = 1,nx/2
                  do y=1,nyp
                     uxy(x,y,i,2) = boxr(x,1,y) + im*boxi(x,1,y)
                  enddo
               enddo
            end if
         end do
...
c     call putxy(uxy(1,1,i,1),1,i,ur)
            call putxy(boxr,boxi,1,i,ur,ui)
c            call putxyp(uxy(1,1,i,1),z,i,ur)
            if (nfzsym.eq.0) then 
c     call putxyp(uxy(1,1,i,2),zn,i,ur)
               do x = 1,nx/2
                  do y=1,nyp
                     boxr(x,1,y)=real(uxy(x,y,i,1))
                     boxi(x,1,y)=aimag(uxy(x,y,i,1))
