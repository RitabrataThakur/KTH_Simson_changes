...
c ***********************************************************************
      ...
      subroutine varvisc_32(taurvis,tauivis,ivarvisc,yb,alfa,beta,u0low,
     &     w0low,prexn,prezn,presn,prean,prex,prez,my_node,realg1,
     &     realg2,ur,ui,re,eta,it,cr,ci,nur,nui,tauplr,taupli,s2r,s2i,
     &     l2r,l2i,m2r,m2i,om2r,om2i,du2r,du2i,u2r,u2i,delT,Tl,c1,
     &     xl,zl,suth)
...
c
            call getxz(cr(1,1),ci(1,1),yb,
     &           scalf,0,ur,ui)
c            call getxz(th2r(1,1,3+4*(ith-1)),th2i(1,1,3+4*(ith-1)),yb,
c     &           9+pressure+3*(ith-1),1,ur,ui)
c         end do
      else
#ifdef MPI
         do i=1,3
     ...
     &        realg1,realg2,my_node)
c         do ith=1,scalar
            call getpxz(cr(1,1),ci(1,1),yb,
     &           scalf,0,ur,ui,
     &           realg1,realg2,my_node)
c            call getpxz(th2r(1,1,3+4*(ith-1)),th2i(1,1,3+4*(ith-1)),yb,
c     &           9+pressure+3*(ith-1),1,ur,ui,
...
c$$$         do i=1,3
c$$$            call getxz(u2r(1,1,i),u2i(1,1,i),yb,i,0,ur,ui)
c$$$         end do
c$$$         call getxz(om2r(1,1,1),om2i(1,1,1),yb,4,0,ur,ui)
c$$$         call getxz(om2r(1,1,3),om2i(1,1,3),yb,5,0,ur,ui)
c$$$         call getxz(cr(1,1),ci(1,1),yb,scalf,0,ur,ui)
c$$$      else
c$$$#ifdef MPI
c$$$         do i=1,3
c$$$            call getpxz(u2r(1,1,i),u2i(1,1,i),yb,i,0,ur,ui,
c$$$     &           realg1,realg2,my_node)
c$$$         end do
c$$$         call getpxz(om2r(1,1,1),om2i(1,1,1),yb,4,0,ur,ui,
c$$$     &        realg1,realg2,my_node)
c$$$         call getpxz(om2r(1,1,3),om2i(1,1,3),yb,5,0,ur,ui,
c$$$     &        realg1,realg2,my_node)
c$$$         call getpxz(cr(1,1),ci(1,1),yb,scalf,0,
c$$$     &        ur,ui,realg1,realg2,my_node)         
c$$$#endif
c$$$      end if

c      if(my_node.eq.0) then
c         write(*,*) '##################'
c         write(*,*) 'after. Here at the end', yb, my_node
c         write(*,*) '##################'
c      endif
...
c         c1 = (Tref+110.4)/(Tref*sqrt(Tref))
         do z=1,nz
            do x=1,nx/2
               temp = delT*cr(x,z) + Tl
               nur(x,z) = (c1*temp*sqrt(temp))/(temp+110.4) - 1.0
               
               temp = delT*ci(x,z) + Tl
               nui(x,z) = (c1*temp*sqrt(temp))/(temp+110.4) - 1.0
         ...
         do y=1,npl
            ...
            do z=nz/2+1,nz
               do x=1,nx/2
                  xy=x+(y-1)*(nxp/2+1)
                  zz = z+nzp-nz
                  l2r(xy,z,ll) = m2r(xy,zz,ll)*gd2/re
                  l2i(xy,z,ll) = m2i(xy,zz,ll)*gd2/re
               end do
            end do
         end do
      end do
...
c      write(*,*) '##################'
c      write(*,*) 'Here at the end'
c      write(*,*) '##################'

c
c     Save stress due in inhomogeneous viscosity. 
c
314   format(f12.6)
