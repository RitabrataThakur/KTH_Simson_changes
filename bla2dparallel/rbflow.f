c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/rbflow.f $
...
c
      ...
      integer fltype
      real u0lowb,w0lowb
      real re,xl,zl,t,xs,dstar,bstart,bslope,spanv,rlam
      logical pou
      real urx(nx)
...
c
      ...
      if (fltype.ne.3.and.(fltype.lt.6.or.fltype.gt.8)) then
         write(*,*) 'The base flow file does not contain'
         write(*,*) 'the correct type of flow'
         stop
      end if
      if (fltype.ge.4.and.fltype.le.6) then
         read(112) bstart,bslope
         rlam=0.0
         spanv=0.0
      end if
      ...
      if (fltype.ge.7) read(112) bstart,bslope,rlam,spanv
