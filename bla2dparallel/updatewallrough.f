c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/updatewallrough.f $
c $LastChangedDate: 2007-10-29 14:23:13 +0100 (Mon, 29 Oct 2007) $
c $LastChangedBy: qiang@MECH.KTH.SE $
c $LastChangedRevision: 786 $
c
c ***********************************************************************
      subroutine updatewallrough(ur,ui,xsc,xst,xlen,delx,zl,zsc,it,
     &     prexn,prey,prezn,presn,prean,alfa,beta,
     &     h_rough,xstart,xend,xrise,xfall,zfunc,q,tpsi,
     &     v_wall,taylor4,monitor,unit1,my_node,
     &     wallur,wallui,wlvr,wlvi,wallwr,wallwi)
c
...
c     Local variables
c
      integer i,x,xsh,xind,y,z,zb,zm,zst,zen,npl,nxy
      integer halfproc
      real delty,zc,step,xc11,xc22,norm
      real zshift1,zshift2,tmpr1,tmpr2
      
      real fx1(lenx),fx2(lenx)
      ...
      if (my_node.eq.0) then
         write(*,*)'--> Wall-roughness B.C.s are updated.'
      end if
c
...
c
c     Write u,v,w value (at hmax) on screen (and into a file?)
c
      if (zfunc.eq.1) then
         if (q.ne.0)   zm = nz/(4*q)+nzn
      else
         zm = 1
      end if
      halfproc = (zm-1)/memnz
      if (my_node .eq. halfproc) then
         x = 0.5*(2*xsh+xlen-0.25)
         z = zm-halfproc*memnz
         write(*,*)'--> u,v,w|0 at h_max:'
         write(*,76) lwallur(x,z),lwallvr(x,z),lwallwr(x,z)

         if (monitor) then
            write(unit1,77)it,lwallur(x,z),lwallvr(x,z),lwallwr(x,z)
         end if
      end if
 76   format(3F22.16)
 77   format(I8.5,3F20.16)
c
