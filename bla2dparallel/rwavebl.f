c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/rwavebl.f $
...
c
      ...
      if (ampob.ne.0.) then
         ...
         call stopnow(5462345)
c
c     Reads eigenmode for oblique waves from the file wave.d
c
         open(unit=10,status='old',file='wave.d')
         read(10,*) rew
         if (ampob.gt.0.) then
            read(10,*) alfac,betaw
         else
...
c
 ...
 7778    continue
         ...
         read(10) vers
         if (my_node_world.eq.0) then
            write(*,*) '* Version of wave file: ',-vers
         end if
         if (vers.gt.-4) then
            write(*,*) 'Impossible to process due to version ',-vers
         end if
         read(10) n, re, ymax
         ...
         if (n+1.ne.nyp) then
            write(*,*) 'Wrong dimension of ',trim(osfil)
            write(*,*) n+1,nyp
            stop
         end if
         
         if (numb.gt.osnf) then
            write(*,*) 'Increase osnf in par.f'
            write(*,*) 'Number of eigenfunctions ',numb
            write(*,*) 'In bla ',osnf
            stop
         end if
c
 ...
 8889    continue
     ...
     &           oswi(jj,i),i=1,osn+1)
            ...
            if ((yyy(1).ne.osymax).or.(yyy(osn+1).ne.0.)) then
               write(*,*) 'Wall-normal direction wrong.'
               stop
            end if
            call checki(n, osn)
            call checkr(re, osre)
...
c            if (abs(modd(osbeta(jj),kzf)).gt.4.*epsilon(kzf)) then
            if (abs(modd(osbeta(jj),kzf)).gt.1e-6) then
               write(*,*) modd(osbeta(jj),kzf),'>',4.*epsilon(kzf)
               write(*,*) 'osbeta(jj) = ',osbeta(jj),'(',jj,')'
               write(*,*) 'not multiple of fundamental wavenumber ',kzf
            end if
c
...
c
         ...
         close(10)
         if (my_node_world.eq.0) then
            write(*,*) '* Total    ',numb,' eigenfunctions.'
            write(*,*) '* Retained ',osnumb,' eigenfunctions.'
            write(*,*) '* OSDAMP   ',osdamp
            write(*,*) '* Generated: ',time
            write(*,*) '* Eigenmodes finished.'
         end if
