c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/rbla.f $
c $LastChangedDate: 2010-07-05 14:32:02 +0200 (Mon, 05 Jul 2010) $
c $LastChangedBy: ilak@MECH.KTH.SE $
c $LastChangedRevision: 1483 $
c
c ***********************************************************************
      subroutine rbla(fbla,nbla,dybla,
     &     rlam,dstar2,fltype,m1,pr,my_node_world,x0,re,dstar,thgrad)
c
...
c
      ...
      if (my_node_world.eq.0) then
         write(*,*) ''
         write(*,*) 'Reading base-flow profile '
         write(*,*) '  eta (start) : ',a
         write(*,*) '  eta (end)   : ',b
...
c
      if(nbla.gt.mbla) then
         if (my_node_world.eq.0) then
            write(*,*) 'The base-flow table is too large.'
            write(*,*) 'Increase mbla in par.f to at least ',nbla,
     &           ' (now: ',mbla,')'
            write(*,*) 'and recompile'
         end if
         recreate = .true.
      ...
      if (scalar.eq.1.and.scalarin.eq.0) then
         if (my_node_world.eq.0) then
            write(*,*) 'Scalar flag not the same:'
      ...
      do j=1,scalar
         if (pr(j).ne.prin(j)) then
            if (my_node_world.eq.0) then
               write(*,*) 'The flow field and the flow-profile file'
               write(*,*) 'do not have the same Prandtl number'
               write(*,*) 'Scalar nr: ',j
            end if
            recreate = .true.
         end if
         if (m1(j).ne.m1in(j)) then
            if (my_node_world.eq.0) then
               write(*,*) 'The flow field and the flow-profile file'
               write(*,*) 'do not have the same exponent for the scalar'
               write(*,*) 'Scalar nr: ',j
            end if
            recreate = .true.
         end if
      ...
      if (a.ne.0.) then
         write(*,*) 'Data should start at eta=0'
         recreate = .true.
      end if
...
c     
         if (my_node_world.eq.0) then
            write(*,*) '** Please provide suitable base flow in fsc.dat'
         end if
         call stopnow(123554)
