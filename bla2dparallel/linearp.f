c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/linearp.f $
...
c ***********************************************************************
     ...
     &     vr,vi,d2vr,d2vi,
     &     hvr,hvi,
     &     q,c,d,e,w3,rv1r,rv1i,wd1,it,dtn,my_node_world)
c
...
c
      ...
      real bbeta(nx/2*mbz),k2(nx/2*mbz),bc(nxz,2,2),c4
c     
...
c     
      call dcheb(fvr,h3r(1,1,2),ny,nxz,nxz)
      call dcheb(fvi,h3i(1,1,2),ny,nxz,nxz)
c     
...
c
         ...
         do xz=1,nxz
            x=xz+xb
            pl=(xz-1)/(nx/2)
            vr(xz,1)=vr(xz,1)+(d2vr(xz,1)-k2(x)*u3r(xz,1,2))/re
     &           -(u3r(xz,1,2)-rv1r(x-nx/2*pl,zb+pl,1))*c4
            vr(xz,2)=vr(xz,2)+(d2vr(xz,2)-k2(x)*u3r(xz,nyp,2))/re
     &           -(u3r(xz,nyp,2)-rv1r(x-nx/2*pl,zb+pl,2))*c4
            vi(xz,1)=vi(xz,1)+(d2vi(xz,1)-k2(x)*u3i(xz,1,2))/re
     &           -(u3i(xz,1,2)-rv1i(x-nx/2*pl,zb+pl,1))*c4
            vi(xz,2)=vi(xz,2)+(d2vi(xz,2)-k2(x)*u3i(xz,nyp,2))/re
     &           -(u3i(xz,nyp,2)-rv1i(x-nx/2*pl,zb+pl,2))*c4
         end do
c
...
c            x=xz+xb
c            pl=(xz-1)/(nx/2)
c            vr(xz,1)=vr(xz,1)+(d2vr(xz,1)-k2(x)*u3r(xz,1,2))/re
c            vr(xz,2)=vr(xz,2)+(d2vr(xz,2)-k2(x)*u3r(xz,nyp,2))/re
c            vi(xz,1)=vi(xz,1)+(d2vi(xz,1)-k2(x)*u3i(xz,1,2))/re
c            vi(xz,2)=vi(xz,2)+(d2vi(xz,2)-k2(x)*u3i(xz,nyp,2))/re
c         end do
c
...
c     
      call setmac(q,c,d,e,k2(1+xb),ny,nxz,nxz,neumann,cc)
...
c
         ...
         call icheb(temi,temp,bc0,ny,1,1)
c
...
c     
      ...
      do y=1,ny
         do xz=1,nxz
            x=xz+xb    
            om3r(xz,y,1)= om3r(xz,y,1) +bbeta(x)*u3i(xz,y,2)
            om3r(xz,y,3)=-om3r(xz,y,3) -alfa(x) *u3i(xz,y,2)
            om3i(xz,y,1)= om3i(xz,y,1) -bbeta(x)*u3r(xz,y,2)
            om3i(xz,y,3)=-om3i(xz,y,3) +alfa(x) *u3r(xz,y,2)
         end do
      end do
c
