c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/bflow.f $
c $LastChangedDate: 2008-05-06 16:28:35 +0200 (Tue, 06 May 2008) $
c $LastChangedBy: pschlatt@MECH.KTH.SE $
c $LastChangedRevision: 1180 $
c
c ***********************************************************************
      subroutine bflow(bu1,bu2,bu1jr,bu1ji,xs0,x0,eta,
     &     fbla,rlam,m1,spanv,re,xl,dstar,
     &     bstart,bslope,tabfre,namfre,rbfl,nambfl,
     &     u0low,w0low,u0upp,du0upp,wd1,prex,prey,
     &     wbr,wbi,boxr,boxi,w3,fltype,my_node_world,
...
c      
      ...
      integer fltype
      real rlam,m1(scalar),spanv
      real x0,re,xl,dstar,xs0,bstart,bslope,u0low,w0low,du0upp,u0upp
...
c
         if (my_node_world.eq.0) then
            write(*,*) 'Read basic flow from 3D field ',trim(nambfl)
         end if
...
c
     ...
     &        bstart,bslope,tabfre,namfre,u0low,w0low,prex,prey,
     &        wbr,wbi,fltype,my_node_world,suction,asbl,vsuc,nbla,dybla,
     &        thgrad,dtheta0_low,tbc,theta0_low,theta0_upp)
      end if
c
