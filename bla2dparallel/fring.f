c ***********************************************************************
c
c $HeadURL: https://www.mech.kth.se/svn/simson/branches/bla2dparallel/fring.f $
c $LastChangedDate: 2008-09-23 09:08:49 +0200 (Tue, 23 Sep 2008) $
c $LastChangedBy: qiang@MECH.KTH.SE $
c $LastChangedRevision: 1305 $
c
c ***********************************************************************
            ...
            do zp=1,nzd_new/nprocz
               ...
               do y=1,min(mby,nyp-yb+1)
                  ...
                  do x=xst,xen
                     ...
                     c2c=c2c*bz1
                     c2s=c2s*bz1
                     om2r(x,y,zp,1)=om2r(x,y,zp,1)-fring1(x)*(
     &                    -(eur(y1)*c1c-eui(y1)*c1s)
     &                    -(eu2r(y1)*k1c-eu2i(y1)*k1s))
                     om2i(x,y,zp,1)=om2i(x,y,zp,1)-fring2(x)*(
     &                    -(eur(y1)*c2c-eui(y1)*c2s)
     &                    -(eu2r(y1)*k2c-eu2i(y1)*k2s))
                     om2r(x,y,zp,2)=om2r(x,y,zp,2)-fring1(x)*(
     &                    -(evr(y1)*c1c-evi(y1)*c1s)
     &                    -(ev2r(y1)*k1c-ev2i(y1)*k1s))
                     om2i(x,y,zp,2)=om2i(x,y,zp,2)-fring2(x)*(
     &                    -(evr(y1)*c2c-evi(y1)*c2s)
     &                    -(ev2r(y1)*k2c-ev2i(y1)*k2s))
                     om2r(x,y,zp,3)=om2r(x,y,zp,3)-fring1(x)*(
     &                    -(ewr(y1)*c1cw-ewi(y1)*c1sw))
                     om2i(x,y,zp,3)=om2i(x,y,zp,3)-fring2(x)*(
            ...
            do zp=1,nzd_new/nprocz
               ...
               do y=1,min(mby,nyp-yb+1)
                  ...
                  do x=xst,xen
                     ...
                     wdis2=0.
                     do j=1,osnumb
                        udis1=udis1+osur(j,y1)*cphas1(j,x,zp)-
     &                       osui(j,y1)*sphas1(j,x,zp)
                        udis2=udis2+osur(j,y1)*cphas2(j,x,zp)-
     &                       osui(j,y1)*sphas2(j,x,zp)
                        
                        vdis1=vdis1+osvr(j,y1)*cphas1(j,x,zp)-
     &                       osvi(j,y1)*sphas1(j,x,zp)
                        vdis2=vdis2+osvr(j,y1)*cphas2(j,x,zp)-
     &                       osvi(j,y1)*sphas2(j,x,zp)
                        
                        wdis1=wdis1+oswr(j,y1)*cphas1(j,x,zp)-
     &                       oswi(j,y1)*sphas1(j,x,zp)
                        wdis2=wdis2+oswr(j,y1)*cphas2(j,x,zp)-
     &                       oswi(j,y1)*sphas2(j,x,zp)
                     end do
                     om2r(x,y,zp,1)=om2r(x,y,zp,1)-fring1(x)*
     &                    (- udis1)
                     om2i(x,y,zp,1)=om2i(x,y,zp,1)-fring2(x)*
         ...
         if (.not.pert) then
            do zp=1,nzd_new/nprocz
               do y=1,min(mby,nyp-yb+1)
                  y1=y+yb-1
                  do x=xst,xen
                     om2r(x,y,zp,1)=om2r(x,y,zp,1)-
     &                    fring1(x)*(u2r(x,y,zp,1)-bu1(x,y1,1))
                     om2i(x,y,zp,1)=om2i(x,y,zp,1)-
     &                    fring2(x)*(u2i(x,y,zp,1)-bu2(x,y1,1))
                     om2r(x,y,zp,2)=om2r(x,y,zp,2)-
     &                    fring1(x)*(u2r(x,y,zp,2)-bu1(x,y1,2)-vsuc)
                     om2i(x,y,zp,2)=om2i(x,y,zp,2)-
     &                    fring2(x)*(u2i(x,y,zp,2)-bu2(x,y1,2)-vsuc)
                     om2r(x,y,zp,3)=om2r(x,y,zp,3)-
     &                    fring1(x)*(u2r(x,y,zp,3)-bu1(x,y1,3))
                     om2i(x,y,zp,3)=om2i(x,y,zp,3)-
     &                    fring2(x)*(u2i(x,y,zp,3)-bu2(x,y1,3))
                  end do
               end do
            end do
         else
            do zp=1,nzd_new/nprocz
               do y=1,min(mby,nyp-yb+1)
                  ...
                  do x=xst,xen
                     om2r(x,y,zp,1)=om2r(x,y,zp,1)-
     &                    fring1(x)*(u2r(x,y,zp,1))
                     om2i(x,y,zp,1)=om2i(x,y,zp,1)-
     &                    fring2(x)*(u2i(x,y,zp,1))
                     om2r(x,y,zp,2)=om2r(x,y,zp,2)-
     &                    fring1(x)*(u2r(x,y,zp,2))
                     om2i(x,y,zp,2)=om2i(x,y,zp,2)-
     &                    fring2(x)*(u2i(x,y,zp,2))
                     om2r(x,y,zp,3)=om2r(x,y,zp,3)-
     &                    fring1(x)*(u2r(x,y,zp,3))
                     om2i(x,y,zp,3)=om2i(x,y,zp,3)-
     &                    fring2(x)*(u2i(x,y,zp,3))
                  end do
               end do
            end do
         end if
...
c
     ...
     &                    fring1(x)*(th2r(x,y,zp,1+4*(ith-1))-
     &                    bu1(x,y1,4+(ith-1)))
                     th2i(x,y,zp,4+4*(ith-1))=th2i(x,y,zp,4+4*(ith-1))-
     &                    fring2(x)*(th2i(x,y,zp,1+4*(ith-1))-
     &                    bu2(x,y1,4+(ith-1)))
                  end do
               end do
            end do
